...
- **主题不是可删组件**：`ui_pixel` 主题体系（天空底色、通栏状态栏、草地、吉祥物、整页墨色描边面板）属于用户界面的一部分，不是可删除的"不必要组件"。精简组件或直接载入功能界面时，保留该主题，并通过 `ui_pixel_screen_create()` / `ui_pixel_panel_create()` 建屏。
- **默认右上角电量**：用户界面在没有默认要求的情况下，默认在**状态栏右上角显示电量信息**；除非开发者明确指定其它位置或明确不需要。电量读取 `bsp_battery_soc()`（需要时用 `bsp_battery_mv()`），以小型电池图标或百分比呈现；读值为 `-1`（不可用）时优雅降级，不要画一个数字。页面标题必须在状态栏居中，白云放在状态栏下方，避免互相遮挡。
